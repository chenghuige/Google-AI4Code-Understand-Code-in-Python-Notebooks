...
def main(_): 
  ...
  for fold in range(folds):
    ...
    model_dir = model_dirs[0]
    l = []
    ensembler = gezi.Ensembler(includes=['pred', 'cls_pred', 'reg_pred'])
    for i, x in tqdm(enumerate(xs), total=len(xs), desc='convert', leave=False, ascii=True):
    # for i, x in enumerate(xs):
      x = {k: v  for k, v in x.items() if k in ['pred', 'cls_pred', 'reg_pred', 'id', 'cell_id']}
      try:
        metrics_file = f'{model_dirs[i]}/metrics.csv'
        d = pd.read_csv(metrics_file)
        metrics = d.to_dict('records')[-1]
        metrics = {k: v for k, v in metrics.items() if k in FLAGS.show_keys}
        ...
        pass
      gezi.restore_configs(model_dirs[i], ignores=['show_keys', 'num_ids'])
  ...
  if FLAGS.ensemble_kaggle:
    ic(model_dirs_)
    if mark == 'online':
      for i, model_dir in enumerate(model_dirs_):
        kaggle_idx = FLAGS.kaggle_idx + i
        ic(i, kaggle_idx, model_dir)
        gezi.prepare_kaggle_dataset(MODEL_NAME, model_dir=model_dir, kaggle_idx=kaggle_idx, fold=0, exit_last=False)
    else:
      ...
      for i, model_dir_ in enumerate(model_dirs_):
        for j in range(FLAGS.kaggle_folds):
          ...
          ic(i, j, fold, kaggle_idx, model_dir)
          gezi.prepare_kaggle_dataset(MODEL_NAME, model_dir=model_dir, kaggle_idx=kaggle_idx, fold=0, exit_last=False)
          idx += 1
