...
def main(_):
  ...
  CODE = os.path.basename(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
  ...
  if rank == 0:
    ...
    if FLAGS.kaggle_prepare:
      gezi.prepare_kaggle_dataset(f'{CODE}-model')
  ...
  with strategy.scope():    
    ...
    model.str_keys = ['id', 'source', 'cell_type', 'cell_id'] if FLAGS.tf_dataset else []
    model.out_keys = [] 
    ...
    if FLAGS.tf_dataset:
      ...
      out_hook = None if not FLAGS.pairwise_eval else postprocess.out_hook
      fit(model,  
          dataset=train_ds,
          eval_dataset=eval_ds,
          valid_dataset=valid_ds,
          test_dataset=test_ds,
          # gen_dataset_fn=get_dataloader if (FLAGS.external and FLAGS.external_parts) else None,
          out_hook=out_hook,
          eval_fn=ev.evaluate,
          valid_write_fn=ev.valid_write,
          infer_write_fn=ev.infer_write,
  ...
  if rank == 0:    
    ...
    # gezi.save(gezi.get('eval:embs'), f'{FLAGS.model_dir}/embs.pkl', verbose=True)
    
    ## some time training might fail as last without saving below
    if FLAGS.save_final:
      if FLAGS.online:
        if not FLAGS.torch:
